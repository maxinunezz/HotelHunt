...
export default function FormPageH() {
  ...
  const handleDescripcionChange = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (value === '') {
      useFormStore.setState((state) => ({
        errors: { ...state.errors, descripcion: 'La descripción es obligatoria.' },
      }));
    } else {
      useFormStore.setState((state) => ({
    ...
    <div className="flex h-screen">
      ...
      <div className="w-full bg-gray-800 shadow-lg p-8 overflow-y-auto">
        <div className="max-h-full">
          ...
          <h2 className="text-2xl font-bold mb-4 text-blue-500 text-center">✨REGISTRO DE HOTELES✨</h2>
          {isCreated && <p className="text-green-500">El formulario se creó con éxito.</p>}
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label htmlFor="nombre" className="block text-sm font-medium text-blue-500">
                Nombre:
              </label>
              <input
                type="text"
            ...
            <div>
              <label htmlFor="locacion" className="block text-sm font-medium text-blue-500">
                Locacion:
              </label>
              <input
                type="text"
                ...
                className="w-full border border-gray-300 rounded-md px-4 py-3 mt-1 focus:outline-none focus:ring-blue-500 focus:border-blue-500"
              />
              {errors.locacion && <p className="text-red-500">{errors.locacion}</p>}
            </div>
            <div>
              <label htmlFor="capacidad" className="block text-sm font-medium text-blue-500">
                Capacidad:
              </label>
              <input
                type="number"
            ...
            <div>
              <label htmlFor="descripcion" className="block text-sm font-medium text-blue-500">
                Descripcion:
              </label>
              <textarea
                id="descripcion"
            ...
            <div>
              <label htmlFor="imagen" className="block text-sm font-medium text-blue-500">
                Imagen:
              </label>
                ...
                {imagen ? (
                  <div className="flex items-center justify-between">
                    <div className="w-10/12">
                      <img src={imagen} alt="Imagen seleccionada" className="max-h-full max-w-full" />
                    </div>
                    <button
                      type="button"
